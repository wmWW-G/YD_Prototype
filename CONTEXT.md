...
### 套图/详情内容密度修订（2026-09-16）

- 用户确认按主题自动展开，默认简洁展示仅作视觉风格，不能禁字或减少内容。套图/详情planner系统提示统一为简明规则，要求主题、主体布局、三个不同支持区及准确文案；prepare去除资料不足则仅展示的退化路径。保持原Start/End及图连接，候选v3不变；主图/编辑/同款本轮未修改。
- Dify套图993dd24c-f5c3-4682-9738-58a120934ee7与详情3ae596b4-b6eb-4903-a49c-34e1460385cb草稿通过文字测试和单步规划检查后发布第2版，凭据未改。套图原型相同商品、六图位成功（c0501270-5c66-4bcb-b9ef-768f419496ef）；详情API文字输入两图位成功（26c7253f-b1f5-4702-8dab-7f08345d255e）。28项离线检查通过；结果、实际Prompt、边界见原工作流生成结果.md及validation-results.json.density_revision。
- 内容密度比单特写改善，但不能称质量全通过：底部三分区排版较重复，源图Custom Printed等广告词仍混入，详情结构图额外生成未提供的BluePeak标识且两张详情遗漏标题。未做后期修图。回退目标Git0920ecf2及Dify第1版，当前无技术发布拦截。

### 套图/详情内容选项与版式多样性（2026-09-16）

- src/image-studio.js的BUSINESS_ROLES共用8个新增主题：公司实力、生产制造流程、工厂与设备、质量检测、定制能力、资质认证、仓储与交付、合作流程。默认仍为套图6张/详情8张，上限10张。每张（含套图固定主图）的brief默认展开，可收起，500字符选填；请求仍传slots_json的语义role与brief。手机端补充输入单列。
- build_workflows.py两套planner取消T/L/A/B/C与固定3支持区，改为内容决定信息组织、相邻构图差异、字体色系统一，保留事实约束与完整规划。Dify两应用已发布第3版；Start/End与0920ecf2一致，候选v3未冻结。回退为Dify第2版；当前没有技术发布拦截。
- 验证：196项Node测试、28项Workflow离线检查通过；桌面编辑与390px无横向溢出通过。套图真实文字生成3/3（9a9ab10b-80d7-43f8-878e-f6f6691f346a），详情2/2（ada32a50-993b-4136-ac99-74a889571f32）。测试使用虚构商品/企业资料而非用户事实，5张均目视检查，支持内容驱动的不同构图；4张仍有模型臆造品牌标识的遗留问题，不宣称全部质量通过。证据见工作流生成结果.md和validation-results.json.layout_revision。

### Flare质量档位直连接口测试（2026-09-16）

- 用户要求跳过Workflow直接测试。读取本机忽略文件.env.image-studio.local内API_302_KEY，直接调用302 /v1/images/generations；同一提示词、1024x1024、各1张，low/medium/high/xhigh/max/auto均HTTP200。输出Token依次196/439/1756/3122/7024/1756；所有图片自然尺寸1024x1024，六张已目视检查。只测试文字生成，不能据此宣称图生图所有档位已验证。
- 本轮不修改Workflow参数或发布版本。每档单样本，不作统计排名或实付费用结论。原图、耗时与共用Prompt追加工作流生成结果.md；机器证据在validation-results.json.direct_flare_quality_test。密钥未记录到测试结果或提交文件。
