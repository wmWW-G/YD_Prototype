...
- `成交顾问 > 客户背调顾问`：已从通用聊天壳升级为 `客户背调DeepSeek` 专用原型页，包含 Dify API Key 临时输入框、客户信息输入、快捷样例、背调生成态和结果区；当前支持前端直连 Dify 的内测模式，API Key 仅保存在当前页面内存中，刷新后消失。真实 Dify 调用细节仍维护在 `dify-chatflows/成交顾问-客户背调顾问-客户背调DeepSeek/`。
...
- 当前 `成交顾问 > 客户背调顾问` 为了内测可前端直连 Dify，页面提供 Dify API Key 临时输入框，但 API Key 不落盘、不进 git、不进本地存储。正式产品仍应通过赢单后端代理调用。
