...
- 一句话多目标成交任务必须由客户推进 Runtime 拆解,不能被 `写邮件` 抢成单一开发信。例如 `德国客户嫌贵但想做独家代理，产品是太阳能灯，帮我判断成交策略、写邮件、做报价边界和7天跟进计划` 应返回 `customer-followup-plan` 和 `客户推进分析.md`,执行计划里要出现 `拆解复合任务`,产物里要同时包含 `复合任务拆解`、`成交策略`、`报价边界`、`英文邮件草稿` 和 `7天跟进节奏`;依据里要保留 `产品: 太阳能灯`、`客户/市场: 德国`、`客户关注点: 议价/折扣压力、独家代理/渠道合作`。这类材料仍只是本地草稿,不能自动外发、承诺底价或写入客户档案。
