...
- 「客户开发」在产品信息架构上应作为一级业务入口,不是 `技能Skill` 的子菜单。它负责获客主流程:定义目标市场、找候选客户、筛选评分、生成触达节奏并沉淀到客户Kass;`新客开发信` 才是 `技能Skill` 里的单点写信工具。
...
- `business-draft` adapter:用于开发信草稿、客户推进分析、询盘回复草稿和市场调研这类轻量 Markdown 业务产物。
...
- `workbench/skills/market-research/skill.json`:新对话可生成市场调研 Markdown;用户说 `帮我做德国市场调研，产品是太阳能路灯` 时会输出市场机会、渠道与客户类型、风险和下一步动作;缺产品或目标市场时先追问,不生成泛泛市场报告。
...
- 等待确认期间,如果用户不是确认/取消/补充,而是给出一个完整的新匹配任务,例如外发确认卡还在等待时输入 `帮我生成报价单，产品太阳能路灯，数量500套，单价USD 35，FOB Shanghai`,后端必须切断旧确认卡和旧任务上下文,按新任务启动 Runtime;不能把完整新任务塞进旧 `pendingConfirmation.supplements`。普通补资料和确认语仍继续服务当前确认卡。
...
- pending task 可以自然取消:用户在缺资料等待态说 `算了`、`不用了`、`先不做这个了`、`取消任务` 这类明确放弃当前任务的话,后端返回 `task-cancelled / completed`,清掉 `context.pendingTask`,不调用 Runtime,也不能把取消话术当成 `补充资料` 拼进下一次任务。取消后用户下一句完整任务按干净新任务执行。
- pending task 必须累积多轮补充:用户分多句补客户、询盘、产品、下一步目标时,后端用“原始任务 + 全部 supplements + 当前输入”重新匹配和执行,不能只保留最后一句补充。
- 同一线程里的新匹配任务也要能沿用最近真实用户资料:例如先生成 `开发信草稿.md`,上一轮用户已经说清 `德国采购商 / 太阳能路灯 / MOQ和交期`;下一句 `再做一个客户下一步推进计划` 应把这些用户说过的事实带入 `客户推进分析`,不能像失忆一样重新追问。只允许复用最近用户消息里的事实,不能把 Agent 生成的推测、内部 runId、路径或 tool 信息当作业务事实;也不能把 `发给客户 / 外发 / 先生成草稿 / 确认继续 / 保存 / 导出` 这类操作性确认话当业务资料带进下一轮 Runtime。复用后仍缺资料时继续追问。用户明确说 `另一个客户 / 新客户 / 换个买家` 或英文 `another buyer / new customer / different client` 时必须视为新客户对象,不能套用上一位客户的国家、产品和问题。用户明确说 `重新开始 / 从头开始 / 新任务 / 不要沿用上一个任务` 或英文 `start over / new task / don't reuse previous customer` 时也必须切断旧线程事实、旧 pending task、旧确认卡和旧产物上下文;如果新任务本身缺客户资料或当前卡点,应回到 `needs-input / waiting`,而不是把上一轮客户资料粘过去。但 `不要重新开始 / 先别从头开始 / don't restart` 这类否定重开话术表示继续当前任务,不能误清上下文。
- 可恢复异常也必须保留续接上下文:如果 SSE 或网络在后端返回 sessionId 前中断,前端会生成 `agent-session-local-*` 兜底 ID,并把原始任务写入 `context.pendingTask`;用户下一句补充资料时,后端仍能按同一任务续跑,不能把补充句当成全新任务。若异常发生在已有 `pendingTask.runtime` 的后端 checkpoint 续跑过程中,`buildRecoverableAgentErrorResult()` 必须保留原 `pendingTask.originalText`、`runtime.runId`、`resumeFrom` 和已补 supplements,并把本轮补充追加到 supplements;若异常发生在已有 `pendingConfirmation` 的保存、导出、外发或付费确认续跑过程中,必须保留 `pendingConfirmation` 和当前 `artifact / period / customerSlug`,不能降级成新的 pending task,否则下一句 `继续` 会找不到刚才确认的动作或当前产物。用户在确认句里顺手补的业务事实也要保留,例如 `可以，产品是太阳能路灯，先生成草稿` 应把产品补进 `pendingConfirmation.supplements`,不能因为后端异常只留下按钮确认。公开 payload 仍不能泄露 runId、checkpointPath、`needs-input:<skillId>`、`policy:<action>`、本地路径或 `pendingConfirmation` 内部字段。
- 业务产物缺资料 gate:即使已经匹配到 `开发信草稿`、`客户推进分析`、`询盘回复草稿`、`市场调研` 或 `报价单`,如果缺客户、产品、目标市场、询盘原文、当前卡点、数量、单价或贸易条款等关键上下文,新对话也必须先追问并进入 `waiting`,不能生成空泛产物;这条路径也必须返回可见过程,至少包含 `识别任务 / 核对资料 / 等待补充`。市场调研至少需要产品或核心卖点和目标市场/国家。询盘回复里单独出现 `MOQ/交期` 只能算客户问题,不能冒充产品资料;报价单里单独出现 `报价/价格` 只能算任务意图,不能冒充产品资料;`客户是德国买家` 这类身份描述也不能因为出现“买家”就被当成采购动作;如果没有产品资料或报价边界,必须先追问。报价单里币种和贸易条款要分开判断:`单价20美元` 只说明单价和币种,不能冒充 `FOB/CIF/EXW/DDP/DAP/CFR`、目的港或港口;此时只追问 `贸易条款`。
- 引用资料属于用户提供的业务上下文,不能被当成装饰文本丢掉。前端 `引用资料` 按钮可直接读取 txt/md/csv;XLSX/XLSM 会先调用 `POST /api/agent/reference/parse`,由后端 openpyxl 把前几张工作表整理成人类可读文本后再追加到当前任务草稿。用户说 `帮我做报价单` 并引用 txt/md/csv/xlsx 内容时,后端要从 `产品,太阳能路灯`、`数量,500套`、`单价,USD 35`、`贸易条款,FOB Shanghai` 这类 CSV 行,从 `产品 | 数量 | 单价 | 贸易条款` + `太阳能路灯 | 500套 | USD 35 | FOB Shanghai` 这类 XLSX 横向表格行,以及 `产品 | 太阳能路灯`、`数量 | 500套` 这类 XLSX 纵向键值行里识别产品、数量、单价和贸易条款;字段齐全时直接生成 `报价单.xlsx`,不能继续追问单价或产品资料。PDF 仍不是第一阶段直接引用格式,不要在前端伪装支持。报价/PI 任务里的口语金额也算单价,例如 `帮我做PI，太阳能路灯500套，35美金，FOB上海` 应直接生成报价单;但 `样品费,USD 35`、`样品费,35 USD`、`运费,USD 35` 这类金额不是单价,不能让报价单绕过单价追问。
- 业务草稿产物必须吸收用户补充里的关键业务事实:产品、国家、MOQ、交期、样品时间、议价/折扣压力、让步上限、独代试运行周期、已读不回/客户沉默、客户观望/决策拖延、采购意向/购买意向、付款/账期压力、质量/售后风险、样品/费用压力、物流/运费压力、小单/MOQ压力、独家代理/渠道合作等不能只藏在 `任务来源` 引用里,应进入邮件正文、依据、客户关注点、报价边界或下一步动作;例如 `产品太阳能路灯,想下周先拿样品` 的询盘回复正文要明确出现 solar street lights 和 sample plan for next week,`客户砍价，产品是家具，怎么谈`、`客户要优惠，产品是家具，怎么谈`、`客户说太贵了，产品是家具，帮我写两版WhatsApp跟进话术` 的客户推进分析都要明确出现 `客户关注点: 议价/折扣压力`,`客户已读不回，产品是家具，怎么跟` 要明确出现 `客户关注点: 客户沉默/未回复`,`客户说再考虑一下，产品是家具，怎么跟` 要明确出现 `客户关注点: 客户观望/决策拖延`,`客户想购买500套太阳能灯，帮我做下一步推进计划` 要明确出现 `客户关注点: 采购意向/购买意向`、`产品: 太阳能灯` 和 `数量: 500套`,`客户要求60天账期，产品是设备，怎么处理` 要明确出现 `客户关注点: 付款/账期压力`,`客户抱怨质量不行，产品是灯具，怎么处理` 要明确出现 `客户关注点: 质量/售后风险`,`客户要免费样品，产品是灯具` 要明确出现 `客户关注点: 样品/费用压力`,`客户嫌运费太贵，产品是灯具，怎么处理` 要明确只归入 `客户关注点: 物流/运费压力`,不能额外混成产品议价压力,`客户只想小批量试单，产品是灯具，怎么处理` 要明确出现 `客户关注点: 小单/MOQ压力`,`客户想做独家代理，产品是灯具，怎么谈` 要明确出现 `客户关注点: 独家代理/渠道合作`;如果复合成交任务首句已经给出 `最多只能让3%` 和 `独代先给3个月试运行`,初稿 `报价边界` 和 `英文邮件草稿` 都要写入这些硬边界。
...
- 询盘回复的“客户问题”不只限于带 `问/询盘` 的原文。用户说 `客户砍价，帮我回一下`、`客户嫌贵，帮我回一封邮件`、`客户要折扣，帮我回一下` 时,已经提供了客户问题;若缺产品,只追问 `产品资料或报价边界`;若同句已有 `产品是家具`,应直接生成 `询盘回复草稿.md`,不能再追问 `询盘原文或客户问题`。
...
- 一句话多目标成交任务必须由客户推进 Runtime 拆解,不能被 `写邮件`、`英文回复` 抢成单一开发信或询盘回复。例如 `德国客户嫌贵但想做独家代理，产品是太阳能灯，帮我判断成交策略、写邮件、做报价边界和7天跟进计划` 或 `这家德国进口商说价格高，还想要独代；产品是太阳能灯，帮我出谈判思路、英文回复、让步范围和下周跟进` 应返回 `customer-followup-plan` 和 `客户推进分析.md`,执行计划里要出现 `拆解复合任务`,产物里要同时包含 `复合任务拆解`、`成交策略`、`报价边界`、`英文邮件草稿` 和跟进动作;依据里要保留 `产品: 太阳能灯`、`客户/市场: 德国`、`客户关注点: 议价/折扣压力、独家代理/渠道合作`。这类材料仍只是本地草稿,不能自动外发、承诺底价或写入客户档案。复合成交识别里的客户对象不只限于“客户”,还包括进口商、经销商、分销商、代理商、这家公司等真实外贸说法。
- 客户分析类请求必须防止空泛生成:`客户是德国采购商,帮我做客户分析`、`客户是德国买家,帮我做客户分析`、`客户是德国采购商,产品是太阳能灯,帮我做客户分析`、`这个买家怎么推进`、`帮我判断这个客户优先级` 这类只有客户类型、产品或任务意图的输入,应先追问询盘、聊天记录或当前卡点;`有个询盘`、`有需求` 这种空壳词也不能算足够依据;补一句 `他问MOQ和交期,产品是太阳能灯` 后再接着同一任务生成 `客户推进分析.md`。但 `客户说/客户问/客户提到/客户要求/客户抱怨/客户投诉 + 具体问题或异议 + 产品/业务目标` 已经是足够的当前卡点,例如 `客户说价格太高，帮我想下一步怎么谈，产品太阳能路灯` 应直接生成客户推进分析,不能继续机械追问客户类型。议价/折扣/砍价这类推进分析必须知道产品或核心卖点;`客户嫌贵，怎么谈`、`买家要折扣，下一步怎么办`、`客户想便宜点，怎么谈` 只能先追问 `产品或核心卖点`,不能生成泛泛谈判建议。业务员常用的代称、议价口语、沉默跟进口语、观望/拖延决策口语、采购意向口语、付款口语、售后口语、样品费口语、小单试单口语、渠道合作口语和验厂/资质审核口语也要识别,例如 `买家嫌贵，产品是家具，帮我想下一步怎么谈`、`对方嫌贵，产品是家具，下一步怎么推进`、`客户砍价，产品是家具，怎么谈`、`客户要优惠，产品是家具，怎么谈`、`买家要折扣，产品是太阳能灯，怎么谈`、`客户已读不回，产品是家具，怎么跟`、`买家一直没回复，产品太阳能灯，下一步怎么办`、`客户说再考虑一下，产品是家具，怎么跟`、`买家说先看看，产品是太阳能灯，下一步怎么办`、`客户想购买500套太阳能灯，帮我做下一步推进计划`、`客户想买500套太阳能灯，帮我做下一步推进计划`、`客户要求60天账期，产品是设备，怎么处理`、`客户抱怨质量不行，产品是灯具，怎么处理`、`客户要免费样品，产品是灯具`、`客户只想小批量试单，产品是灯具，怎么处理`、`客户想做独家代理，产品是灯具，怎么谈`、`客户要验厂，产品太阳能灯，下一步怎么推进` 应直接进入客户推进分析,不能因为没写“客户说”就追问。观望/拖延决策词必须能看出是客户、买家、对方的原话或反馈;`我先看看`、`之后再说` 这类用户自己的操作语气不能冒充客户当前卡点。
...
- 复合成交 Markdown 续改必须改到对应章节正文:当前产物是 `客户推进分析.md` 且包含 `报价边界`、`英文邮件草稿` 等章节时,用户补 `把报价边界改成最多让3%，独代先给3个月试运行，英文邮件语气更坚定一点`,后端必须把 `最多让 3%`、`3个月试运行` 写进 `报价边界`,并把更坚定、边界更清楚的英文段落写进 `英文邮件草稿`;不能只在文末追加 `本次补充优化` 记录。
...
- 同任务 follow-up 必须区分“解释当前产物”和“修改当前产物”:用户问 `为什么这里要强调 MOQ 和交期?`、`为什么这封开发信写成这样?`、`这是什么意思?`、`解释一下这段` 时,后端只读取当前产物和线程上下文回答,返回 `followup / completed` 但 `artifact: null`,不写回 Markdown/XLSX,也不追加 `本次补充优化`;只有 `改一下 / 加一句 / 重写 / 更礼貌 / 加一列` 这类明确修改要求才续改产物。
- 同任务 follow-up 的最终用户可见消息不能出现 `前端 / 日志 / 脚本 / run log / manifest` 等内部实现词;上一轮依据要翻成业务口吻,例如“已沿用当前线程里的产物和上下文继续处理”。
- 同任务 follow-up 要能把用户补充转成真实可用内容,不能只记录“用户要求”。例如当前产物是带 `7天跟进节奏` 的 `客户推进分析.md` 时,用户补 `把第1天/第3天话术写成英文 WhatsApp 和邮件两版`,后端应在同一 Markdown 里追加对应天数的 WhatsApp 和 Email 草稿,包含 Subject 和产品语境;用户补 `第5天` 或 `Day 5` 也应生成对应话术,不能只支持第1/3/7天;用户没指定天数、只说 `写一条 WhatsApp 跟进消息` 时,也应生成一条独立 WhatsApp 跟进消息草稿,而不是只记录要求;这仍只是草稿,不会自动外发。
- 渠道草稿和外发动作必须区分:用户说 `WhatsApp / 邮件 / Email` 不等于要发送;如果同时出现 `话术 / 文案 / 草稿 / 两版 / 写成 / 消息 / 内容` 这类版本编辑词,应按同任务 follow-up 续改当前产物。只有明确说 `发给客户 / 发送给买家 / 外发 / 直接发 / 发到客户 / 然后发送 / 然后发客户 / 发一下 / 发下 / 发过去 / send it / send now / send this to the customer / send this email to the buyer` 时,才进入外发确认。`用于开发客户` 这类业务用途不是外发动作,不能因为包含 `发客户` 三个字就误拦。
- 渠道消息分析和渠道草稿也必须区分:用户说 `帮我分析这段 WhatsApp 消息内容` 是分析已有材料,不能续写成 `WhatsApp Follow-up Message`,也不能因为 `WhatsApp` 弹外发确认;如果要写草稿,通常应出现 `写 / 准备 / 生成 / 起草 / 润色 / message draft` 等创作或修改意图。
...
- `server/agent-reference-parser.mjs` 和 `POST /api/agent/reference/parse`:只用于把用户主动选择的 XLSX/XLSM 引用资料抽成有边界的文本片段,供新对话任务理解报价表、产品表或客户清单。接口不写入客户档案、不创建产物、不返回本机路径;文件过大、PDF 或解析失败时只返回业务化错误文案。
...
- 首轮或补资料续跑里的保存/导出副作用词只能进入确认卡,不能进入 Runtime 业务产物输入。比如生成报价单时,Runtime 输入应保留 `帮我生成报价单 / 产品 / 数量 / 单价 / FOB`,但不能包含 `导出文件`、`保存一下`、`保存到客户档案 Global Sourcing Inc`;确认卡的 `originalText` 仍保留用户原句,用于确认和恢复。
- 旧线程已有 artifact 时也要先判断本轮是不是新的业务产物任务:如果用户说 `帮我生成报价单并导出文件...`、`帮我生成报价单并保存一下...` 或 `德国客户嫌贵，想申请独家代理，产品是太阳能灯，帮我判断成交策略、报价边界和7天跟进节奏，并导出文件`,不能因为 session 里挂着旧 `客户推进分析.md` 就直接确认导出/保存旧文件;必须先生成本轮新的 `报价单.xlsx` 或 `客户推进分析.md`,再进入导出/保存确认。即使用户没说“生成/做/写”,只要去掉保存/导出词后仍是明确业务任务和具体业务上下文,也要先生成新产物。只有 `导出文件`、`导出报价单`、`导出一份报价单`、`保存当前`、`保存当前文件`、`把当前报价单写入客户档案` 这类纯当前产物动作才直接对当前 artifact 弹确认。
...
- 外发确认后的 Runtime 输入必须剥离 `发给 / 发送 / 外发 / send to` 这类副作用动作词,只保留客户、产品、询盘、邮件目标等业务事实。比如 `帮我写一封开发信发给德国客户...` 确认 `先生成草稿` 后,Runtime 应看到开发信、德国客户、太阳能路灯、MOQ/交期,不能再看到 `发给客户`。
...
- 确认卡下面的极短自然回复也应可用:用户只回 `可以`、`好的`、`继续吧`、`没问题` 这类短确认时,应按当前待确认动作继续执行保存、导出、外发草稿或付费继续;但 `继续优化`、`可以更正式一点`、`好像先生成草稿更合适` 这类仍是补充/试探/续改,不能自动触发风险动作。
...
- 付费确认后的 Runtime 输入必须剥离 `调用收费接口 / 付费 / 扣费 / 消耗额度` 这类付费动作词,也不能把 `确认继续` 残留成 `补充资料: 继续`。确认只表示继续整理不扣费部分;真正再次产生费用时仍走 Runtime policy。
...
- 新对话 `引用资料` 不能是空按钮:第一阶段支持选择 `.txt/.md/.csv` 文本资料,并支持 `.xlsx/.xlsm` 通过后端解析成有边界的表格文本;内容以 `引用资料：` 的自然语言块追加进当前输入框并随下一次任务一起发送。前端单份资料会截断在前 6000 字符并提示已截断;后端 XLSX 解析也必须限制文件大小、单元格长度和总输出长度。不支持的 PDF 等文件必须提示先转成 `txt/md/csv/xlsx` 或直接粘贴关键内容,不能把二进制内容塞进输入框,也不能假装已经解析。
...
- `server/agent-customer-memory.mjs`:写入客户档案确认后,只把当前 session 绑定产物的摘要写入 `workbench/customers/<customerSlug>/memory.md`,并追加 `diary/agent-saves.jsonl`。`customerSlug` 必须来自当前客户上下文或用户明确补充;如果用户在保存请求里已写明 `保存到客户档案 global-sourcing-inc` 或 `保存到客户档案 Global Sourcing Inc`,确认卡要带上规范化后的 `global-sourcing-inc` 作为客户标识,但仍必须等用户确认后才写入;没有客户绑定时必须继续追问写入哪个客户档案,不能默认写入 `global-sourcing-inc`。
...
「客户开发」不是这两种 Skill:它是产品一级入口,用于承载找客户和获客推进;它可以调用开发信草稿、客户背调、客户Kass 等能力,但自身不要被归类为 `技能Skill` 子菜单。
```
...
→ 返回 customer-followup-plan 和 客户推进分析.md,产物应包含 `客户关注点: 议价/折扣压力` 和太阳能路灯
输入: 客户要优惠，产品是家具，怎么谈
→ 返回 customer-followup-plan 和 客户推进分析.md,产物应包含 `客户关注点: 议价/折扣压力`
输入: 客户嫌贵，怎么谈
...
输入: 德国客户嫌贵但想做独家代理，产品是太阳能灯，帮我判断成交策略、写邮件、做报价边界和7天跟进计划
→ 返回 customer-followup-plan 和 客户推进分析.md,计划应包含 `拆解复合任务`,产物应包含 `复合任务拆解`、`成交策略`、`报价边界`、`英文邮件草稿`、`7天跟进节奏`,不能被路由成单一开发信
输入: 德国客户嫌贵但想做独家代理，产品是太阳能灯，最多只能让3%，独代先给3个月试运行，帮我判断成交策略、写邮件、做报价边界和7天跟进计划
→ 返回 customer-followup-plan 和 客户推进分析.md,`报价边界` 包含 `最多让 3%` 和 `3个月试运行`,`英文邮件草稿` 包含 `maximum 3% concession` 和 `3-month agency trial period`
...
输入: 客户问报价，产品太阳能路灯，数量500套，单价20美元，帮我做报价单
→ 返回 needs-input / waiting,任务标题为 报价单,只追问 贸易条款
同一个 session 输入: FOB深圳
...
输入: 保存到客户档案 Global Sourcing Inc
→ 如果当前已有业务产物,返回 confirmation-required / customer_write,确认卡带规范化后的 `global-sourcing-inc`,但仍等待用户确认
输入: 确认写入
...
同一个 session 输入: 为什么这里要强调 MOQ 和交期？
→ 返回 followup,artifact 为空,只解释不改写当前 开发信草稿.md
→ 预览接口内容不新增 本次补充优化
输入: 客户已读不回，产品是家具，帮我做一个7天跟进计划
→ 返回 客户推进分析.md
...
输入: 德国客户嫌贵但想做独家代理，产品是太阳能灯，帮我判断成交策略、写邮件、做报价边界和7天跟进计划
→ 返回 客户推进分析.md
同一个 session 输入: 把报价边界改成最多让3%，独代先给3个月试运行，英文邮件语气更坚定一点
→ 返回 followup,artifact 仍是同一份 客户推进分析.md
→ 预览接口的 报价边界 章节包含 最多让 3% 和 3个月试运行,英文邮件草稿 章节包含 firm/clear 语气和 3-month agency trial period
```
