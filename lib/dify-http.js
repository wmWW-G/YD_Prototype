...
/**
 * 初始化浏览器可立即消费的 SSE 响应头。
 *
 * `no-transform` 和 `X-Accel-Buffering: no` 用于尽量阻止 CDN/代理把小事件攒到最后一次性返回。
 *
 * @param {object} res - Vercel Node 响应对象。
 * @returns {void}
 * @throws {Error} 底层响应对象拒绝写响应头时可能抛出。
 */
...
/**
 * 写入一条只有 data 字段的 SSE JSON 事件。
 *
 * @param {object} res - 已由 startSse 初始化的响应对象。
 * @param {object} payload - 可序列化的公开事件。
 * @returns {void}
 * @throws {Error} JSON 序列化或网络写入失败时抛出。
 */
...
function getStatusForError(error) {
  ...
  if (/请输入|请选择|功能标识|Skill ID|必须以 app-|实际是|暂不支持/.test(message)) return 400;
