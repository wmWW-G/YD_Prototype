...
- 维护状态：已完成 2026-07-04 API 连通性测试，并已接入 `成交顾问 > 客户背调顾问` 原型页；当前默认通过 Vercel Serverless 代理调用 Dify，`?difyDebug=1` 仅用于前端直连调试
- 最近更新：2026-07-06
...
当前记录已经对应主静态原型里的 `成交顾问 > 客户背调顾问` 页面。当前原型页默认通过 Vercel Serverless 代理调用 Dify，页面不需要用户填写 Dify API Key。只有 URL 带 `?difyDebug=1` 时，才显示 Dify API Key 临时输入框用于前端直连调试。正式接入时仍建议由赢单后端代理调用 Dify。
...
| 2026-07-04 | v0.4 | 将运行时 prompt 改为页面内 Dify API Key 临时输入框，避免浏览器弹窗和旧缓存造成误判 | 用户反馈线上没有提示填 key |
| 2026-07-06 | v0.5 | 补充 `developer-handoff.md` 开发交接，并同步当前默认代理、调试模式直连的实现口径 | 用户要求发给开发同事 |
